...
17. [RLS / SECURITY] | RLSPolicies.txt, DatabaseSchema.txt, lib/services/classService.ts
    ...
    RESIDUAL (separate efforts, NOT this migration): per-section row scoping of scores/students (needs exam-flow browser reads migrated like #5 first — currently any active staff can read all scores/students via PostgREST); exams.answer_key column-level hiding (#16 residual); users stays readable by all active staff (names + must_change_password) — acceptable.
