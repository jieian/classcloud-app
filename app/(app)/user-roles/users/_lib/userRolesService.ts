...
export async function fetchPendingUsers(): Promise<PendingUser[]> {
  const response = await fetch("/api/users/pending-with-email", {
    method: "GET",
    cache: "no-store",
  });
  ...
  if (!response.ok) {
    if (process.env.NODE_ENV === "development") {
      console.error("Error fetching pending users:", result?.error ?? result);
    }
...
export async function fetchActiveUsersWithRoles(): Promise<UserWithRoles[]> {
  try {
    ...
    const response = await fetch("/api/users/active-with-roles", {
      method: "GET",
      cache: "no-store",
    });
    ...
    if (!response.ok) {
      if (process.env.NODE_ENV === "development") {
        console.error("Error fetching users with roles:", result?.error ?? result);
      }
      throw new Error(result?.error || "Failed to fetch users with roles.");
    }

    const usersWithRoles: UserWithRoles[] = ((result?.data as UserWithRoles[]) || []).map((user: UserWithRoles) => ({
      uid: user.uid,
