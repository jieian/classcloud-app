...
function buildWorkbook({
  ...
  const leftBodyStyle = { font: bodyFont, alignment: left, border };

  setCell("A1", "Individual Exam Result", { font: titleFont, alignment: center });
  ...
  metadata.forEach(([label, value], index) => {
    ...
    const actualRow = index < 4 ? index + 3 : index - 1;
  ...
  const addStudent = (row: ProficiencyRow, index: number) => {
    const values = [
      index,
      row.pupilName,
      `${row.testScore}/${row.totalItems}`,
      `${formatNumber(row.mpl)}%`,
  ...
  const filename = safeFilename(`${gradeName} ${sectionName} - ${subjectName} - ${examTitle} Results.xlsx`);
