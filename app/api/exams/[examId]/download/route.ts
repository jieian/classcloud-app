...
const _GET = async function (
  ...
  if (sectionIds.length === 0) {
    return buildXlsx(exam.title, subjectName, quarterName, sectionNames, examDate, totalItems, generatedBy, generatedOn, [], []);
  }
  ...
  return buildXlsx(exam.title, subjectName, quarterName, sectionNames, examDate, totalItems, generatedBy, generatedOn, males, females);
};
...
function buildXlsx(
  ...
  wsRows[5] = { hpt: 24 };    // Row 6 - metadata 4 (generated on)
  wsRows[6] = { hpt: 24.95 }; // Row 7 - spacer
  wsRows[7] = { hpt: 50.25 }; // Row 8 - column headers
  ...
  const merges: XLSXStyle.Range[] = [
    ...
    { s: { r: 5, c: 1 }, e: { r: 5, c: 7 } }, // B6:H6  generated on value
  ];
  ...
  // Row 6: Generated on
  ws["A6"] = { v: "Generated on:", t: "s", s: { font: labelFont, alignment: leftMid,  border: allBorders } };
  ws["B6"] = { v: generatedOn,     t: "s", s: { font: valueFont, alignment: centerMid, border: allBorders } };
  for (const col of ["C", "D", "E", "F", "G", "H"] as const)
    ws[`${col}6`] = { v: "", t: "s", s: { border: allBorders } };

  // Row 8: Column headers
  ws["A8"] = { v: "LRN",                                       t: "s", s: { font: headerFont, alignment: centerMid,                     border: allBorders } };
  ws["B8"] = { v: "NAME (Last Name, First Name, Middle Name)", t: "s", s: { font: headerFont, alignment: { ...centerMid, wrapText: true }, border: allBorders } };
  ws["C8"] = { v: "Sex (M/F)",                                 t: "s", s: { font: headerFont, alignment: centerMid,                     border: allBorders } };
  ws["D8"] = { v: "Score",                                     t: "s", s: { font: headerFont, alignment: centerMid,                     border: allBorders } };
  ws["E8"] = { v: "Level of Proficiency",                      t: "s", s: { font: headerFont, alignment: { ...centerMid, wrapText: true }, border: allBorders } };

  // ── Data rows ─────────────────────────────────────────────────────────────
  let rowIdx = 8; // 0-based; rowIdx 8 = Excel row 9
