-- Make "Proceed to Reports" atomic and idempotent.

DELETE FROM public.exam_results_reports r
USING public.exam_results_reports newer
WHERE r.exam_id = newer.exam_id
  AND r.section_id = newer.section_id
  AND r.report_id < newer.report_id;

DELETE FROM public.item_analysis_reports r
USING public.item_analysis_reports newer
WHERE r.exam_id = newer.exam_id
  AND r.section_id = newer.section_id
  AND r.report_id < newer.report_id;

CREATE UNIQUE INDEX IF NOT EXISTS uq_exam_results_reports_exam_section
  ON public.exam_results_reports (exam_id, section_id);

CREATE UNIQUE INDEX IF NOT EXISTS uq_item_analysis_reports_exam_section
  ON public.item_analysis_reports (exam_id, section_id);

CREATE OR REPLACE FUNCTION public.create_score(
  p_enrollment_id integer,
  p_exam_assignment_id integer,
  p_responses jsonb,
  p_calculated_score integer,
  p_graded_at timestamp with time zone DEFAULT now()
)
...
BEGIN
  ...
  FOR v_assignment IN SELECT * FROM tmp_finalize_assignments LOOP
    ...
    SELECT
      ...
      correct_responses,
      total_responses,
      COALESCE(ROUND((correct_responses::numeric / NULLIF((SELECT COUNT(*) FROM tmp_report_students), 0)) * 100, 2), 0) AS correct_percent,
      row_number() OVER (ORDER BY correct_responses DESC, item_no ASC) AS rank
    FROM item_rows;
